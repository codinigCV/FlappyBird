...
public class GameBarrierLayerAlter {
   ...
   public GameBarrierLayerAlter(){
       gameTime=new GameTime();
       barriers=new ArrayList<>();
   }
   //绘制障碍物
    public void draw(Graphics g,Bird bird,int BCount){
   /*     Barrier barrier = new Barrier(200, 0, 200, 0);
    ...
    public void logiclarge(Graphics g){    //大量的障碍物
       if(barriers.size()==0){      
          ran();
          gameTime.begin();
      /*    Barrier top=new Barrier(600,0,numberTop,0);
          ...
          insert(600,500-numberDown,numberDown,2);
           ...
           if (differ<=txt)
           {
                g.drawString("最高记录:"+txt+"秒",200,70);
           }else  {
                 setTxt(String.valueOf(differ));
               g.drawString("最高记录:"+getTxt()+"秒",200,70);
    ...
    public void logicmid(Graphics g){    //中量的障碍物
        if(barriers.size()==0){
            ran();
            gameTime.begin();
      /*    Barrier top=new Barrier(600,0,numberTop,0);
   ...
   * 用于得到文件中的数据*/
    ...
    * 用于从池中获取对象，并把参数封装成barrier，存入barrier集合中*/
    ...
    public boolean collideBird(Bird bird){
        BTService<String> btService=new BTService<String>();
        for (int i = 0; i < barriers.size(); i++) {
            Barrier barrier = barriers.get(i);
            /*判断矩形是否相交*/
            if (barrier.getRect().intersects(bird.getRect())){
                System.out.println("collideBird");
                bird.life=false;//先改一下
                //                /*if differ >第五名记录 则是新纪录，new frame新纪录，新纪录输入昵称显示分数*/
                if (gameTime.differ()>btService.findAll().get(4).getTime())
                {
                       new NewRecord().init(gameTime.differ());
                }
                else {
