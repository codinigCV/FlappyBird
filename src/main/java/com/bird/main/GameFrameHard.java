...
public class GameFrameHard extends Frame {
    //示例化参数gamebackground类
    private GamebackGround gamebackGround;
    //实例化Bird类
    private Bird bird;
    //实例化GameBarrierLayerHard类
    private GameBarrierLayerHard gameBarrierLayerHard;
       //示例化GameFrontGround类
    private  GameFrontGround gameFrontGround;
    //存放图片的图片
    private BufferedImage buffimg=new BufferedImage(FRAM_WIDTH,FRAM_HEIGHTH,BufferedImage.TYPE_4BYTE_ABGR);
    public GameFrameHard()  {   //构造器
        //设置 菜单栏
        MenuBar menubar;
        Menu menu, subMenu;
    ...
    public void initGame(){
        ...
        gameBarrierLayerHard=new GameBarrierLayerHard();
    }
    //  
    ...
    public void update(Graphics g) {
        if (bird.life==true){
            ...
            gamebackGround.drawHard(graphics);
            bird.drawSpeed(graphics);
            gameFrontGround.draw(graphics);
            gameBarrierLayerHard.draw(graphics,bird);
            //一次性将图片绘制到屏幕中
